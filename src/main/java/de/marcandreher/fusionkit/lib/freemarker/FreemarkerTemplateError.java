package de.marcandreher.fusionkit.lib.freemarker;
...
public class FreemarkerTemplateError implements TemplateExceptionHandler {
    
    private static final Logger logger = LoggerFactory.getLogger(FreemarkerTemplateError.class);
    private final WebAppConfig config;

    public FreemarkerTemplateError(WebAppConfig config) {
        this.config = config;
    }
     ...
     */
    public static FreemarkerTemplateError create(WebAppConfig config) {
        return new FreemarkerTemplateError(config);
    }
}
